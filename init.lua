...
local ProjectInfra = getgenv().ProjectInfra
...
do
    ...
    Assets.Functions.LoadFile = function(file : string, githublink : string)
        if ProjectInfra.Dev and isfile(file) then
    ...
    Assets.Functions.GetModule = function(name: string)
        if name and ProjectInfra and ProjectInfra.Tabs and ProjectInfra.Tabs.Tabs then
            for i,v in ProjectInfra.Tabs.Tabs do
    ...
    Assets.Functions.GetAllModules = function()
        local modules = {}
        if ProjectInfra and ProjectInfra.Tabs and ProjectInfra.Tabs.Tabs then
            for i,v in ProjectInfra.Tabs.Tabs do
                if v.Modules then
...
do
    Assets.Config.Save = function(File, data)
        writefile("ProjectInfra/Config/"..File..".json", HttpService:JSONEncode(data))
    end
    
    Assets.Config.Load = function(File, set)
        if isfile("ProjectInfra/Config/"..File..".json") then
            local data = readfile("ProjectInfra/Config/"..File..".json")
            local data2 = HttpService:JSONDecode(data)
            if set then
                ProjectInfra.Config[set] = data2
                if set == "Game" then
                    ...
                    if ProjectInfra.Config.Game.Other.TabPos then
                        for i,v in ProjectInfra.Tabs.Tabs do
                            local tabpos = ProjectInfra.Config.Game.Other.TabPos[i]
...
do
    ...
    Assets.Font.Download = function(Name: string, Font: string)
        ...
        if not isfile("ProjectInfra/Assets/Fonts/"..Name..".ttf") then
            if data and not tostring(data):find("404") then
                writefile("ProjectInfra/Assets/Fonts/"..Name..".ttf", data)
            else
                return false
            end
        end
    ...
    Assets.Font.create_family = function(name: string, faces: { FontFace })
        ...
        writefile("ProjectInfra/Assets/Fonts/"..name..".json", HttpService:JSONEncode(family))

        local id = getcustomasset("ProjectInfra/Assets/Fonts/"..name..".json")
...
do
    ...
    Assets.Notifications.Send = function(data: any)
        ...
        for i, v in ProjectInfra.Notifications.Active do
        ...
        NotificationData.Objects.Notification = Instance.new("ImageButton", ProjectInfra.Notifications.Objects.NotificationGui)
        ...
        NotificationData.Functions.Remove = function(anim: boolean)
            if not ProjectInfra or not ProjectInfra.Notifications or not ProjectInfra.Notifications.Active then return end
            for i,v in NotificationData.Connections do
                v:Disconnect()
                if table.find(ProjectInfra.Connections, v) then
                    table.remove(ProjectInfra.Connections, table.find(ProjectInfra.Connections, v))
                end
            end
            ...
            if ProjectInfra and ProjectInfra.Notifications and ProjectInfra.Notifications.Active then
                ProjectInfra.Notifications.Active[flag] = nil
            end
        ...
        table.insert(ProjectInfra.Connections, NotificationData.Connections.conhover)
        table.insert(ProjectInfra.Connections, NotificationData.Connections.unconhover)
        table.insert(ProjectInfra.Connections, NotificationData.Connections.closecon)
        ...
        if ProjectInfra.Notifications.Active[flag] then
            flag = NotificationData.Description..tostring(math.random(0, 1000000000))
            ProjectInfra.Notifications.Active[flag] = NotificationData
        else
            ProjectInfra.Notifications.Active[flag] = NotificationData
        end
        ...
        task.spawn(function()
            repeat 
                TimeLineBar.Size = UDim2.new((os.clock() - start) / ProjectInfra.Notifications.Active[flag].Duration, 0, 0, 2)
                task.wait()
            until ProjectInfra and ProjectInfra.Notifications and ProjectInfra.Notifications.Active[flag] and (os.clock() - start) >= ProjectInfra.Notifications.Active[flag].Duration or ProjectInfra and ProjectInfra.Notifications and ProjectInfra.Notifications.Active and not ProjectInfra.Notifications.Active[flag] or not ProjectInfra or not ProjectInfra.Notifications
            if ProjectInfra and ProjectInfra.Notifications and ProjectInfra.Notifications.Active and ProjectInfra.Notifications.Active[flag] then
...
do    
    Assets.MainBackground.Init = function()
        ...
        ProjectInfra.Notifications.Objects.NotificationGui = Instance.new("ScreenGui", Assets.Functions.gethui())
        ProjectInfra.Notifications.Objects.NotificationGui.ResetOnSpawn = false
        ProjectInfra.Notifications.Objects.NotificationGui.IgnoreGuiInset = true
        ProjectInfra.Notifications.Objects.NotificationGui.DisplayOrder = 10000
        if ProjectInfra.Mobile then
            Instance.new("UIScale", ProjectInfra.Notifications.Objects.NotificationGui).Scale = ProjectInfra.Config.UI.Scale
        end

        ProjectInfra.ArrayList.Objects.ArrayGui = Instance.new("ScreenGui", Assets.Functions.gethui())
        ProjectInfra.ArrayList.Objects.ArrayGui.ResetOnSpawn = false
        ProjectInfra.ArrayList.Objects.ArrayGui.DisplayOrder = 10000
        ProjectInfra.ArrayList.Objects.ArrayGui.Enabled = false
        if ProjectInfra.Config.UI.ArrayList == nil then
            ProjectInfra.Config.UI.ArrayList = false
        end
        ...
        InitInfo.Objects.MainScreenGuiScale.Scale = ProjectInfra.Config.UI.Scale
        ...
        InitInfo.Objects.MainFrame.Position = UDim2.fromScale(ProjectInfra.Config.UI.Position.X, ProjectInfra.Config.UI.Position.Y)
        InitInfo.Objects.MainFrame.Size = UDim2.fromScale(ProjectInfra.Config.UI.Size.X, ProjectInfra.Config.UI.Size.Y)
        ...
        table.insert(ProjectInfra.Corners, mainframecorner)
        ...
        if ProjectInfra.Mobile then
            InitInfo.Objects.ToggleButton = Instance.new("ImageButton", ProjectInfra.Notifications.Objects.NotificationGui)
            ...
            local ProjectInfraIcon = Instance.new("ImageLabel", InitInfo.Objects.ToggleButton)
            ProjectInfraIcon.AnchorPoint = Vector2.new(0.5, 0.5)
            ProjectInfraIcon.BackgroundTransparency = 1
            ProjectInfraIcon.Position = UDim2.fromScale(0.5, 0.5)
            ProjectInfraIcon.Size = UDim2.fromScale(0.5, 0.5)
            ProjectInfraIcon.Image = "rbxassetid://88680068080114"
            Instance.new("UICorner", InitInfo.Objects.ToggleButton).CornerRadius = UDim.new(1, 0)
    
            table.insert(ProjectInfra.Connections, InitInfo.Objects.ToggleButton.MouseButton1Click:Connect(function()
            ...
            InitInfo.Functions.CreateMobileButton = function(info)
                ...
                local dragcon = button.InputBegan:Connect(function(input)
                    if (input.UserInputType == Enum.UserInputType.MouseButton1) or (input.UserInputType == Enum.UserInputType.Touch) then
                        ...
                        ProjectInfra.InputEndFunc = function(input)
                            if (input.UserInputType == Enum.UserInputType.MouseButton1) or (input.UserInputType == Enum.UserInputType.Touch) then
                                local hold = (tick()-HoldTime) >= 0.8
                                MobileButtonInfo.Callbacks.End(MobileButtonInfo, hold)
                                HoldTime = 0
                                ProjectInfra.CurrntInputChangeCallback = function() end
                                ProjectInfra.InputEndFunc = nil
                                ...
                                if hold then
                                    ...
                                    if not ProjectInfra.Config.Game.Other.MobileButtonPos then 
                                        ProjectInfra.Config.Game.Other.MobileButtonPos = {}
                                    end

                                    ProjectInfra.Config.Game.Other.MobileButtonPos[MobileButtonInfo.Flag] = {X = FrameStarting.X.Scale, Y = FrameStarting.Y.Scale}
                                    Assets.Config.Save(ProjectInfra.GameSave, ProjectInfra.Config.Game)
                                end
                            end
                        end
                        ...
                        if HoldTime >= 0.8 then
                            ...
                            ProjectInfra.CurrntInputChangeCallback = function(input)
                                if (input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch) then  
                                    if MobileButtonInfo.Data.Dragging and not ProjectInfra.Config.UI.FullScreen then
                ...
                table.insert(ProjectInfra.Connections, dragcon)
                table.insert(MobileButtonInfo.Connections, dragcon)

                if ProjectInfra.Config.Game.Other.MobileButtonPos and ProjectInfra.Config.Game.Other.MobileButtonPos[MobileButtonInfo.Flag] then
                    local pos = ProjectInfra.Config.Game.Other.MobileButtonPos[MobileButtonInfo.Flag]
                    if pos.X then
        ...
        table.insert(ProjectInfra.Connections, ZoomFrame.MouseWheelForward:Connect(function()
            if UserInputService:IsKeyDown(Enum.KeyCode.LeftControl) or UserInputService:IsKeyDown(Enum.KeyCode.RightControl) and ProjectInfra.Background.Objects.MainFrame.Visible then
                ProjectInfra.Config.UI.Scale = ProjectInfra.Config.UI.Scale + 0.05
                if ProjectInfra.Config.UI.Scale > 3 then
                    ProjectInfra.Config.UI.Scale = 3
                end
                InitInfo.Objects.MainScreenGuiScale.Scale = ProjectInfra.Config.UI.Scale
                Assets.Config.Save("UI", ProjectInfra.Config.UI)
            end
        end))
    
        table.insert(ProjectInfra.Connections, ZoomFrame.MouseWheelBackward:Connect(function()
            if UserInputService:IsKeyDown(Enum.KeyCode.LeftControl) or UserInputService:IsKeyDown(Enum.KeyCode.RightControl) and ProjectInfra.Background.Objects.MainFrame.Visible then
                ProjectInfra.Config.UI.Scale = ProjectInfra.Config.UI.Scale - 0.05
                if ProjectInfra.Config.UI.Scale < 0.4 then
                    ProjectInfra.Config.UI.Scale = 0.4
                end
                InitInfo.Objects.MainScreenGuiScale.Scale = ProjectInfra.Config.UI.Scale
                Assets.Config.Save("UI", ProjectInfra.Config.UI)
            end
        ...
        InitInfo.Functions.CreateNavigationButton = function(Data: any)
            ...
            table.insert(ProjectInfra.Connections, buttondata.Button.MouseEnter:Connect(function()
                hovergradient.Enabled = true
                TweenService:Create(iconscale, TweenInfo.new(0.15), {Scale = 1.2}):Play()
            end))
            table.insert(ProjectInfra.Connections, buttondata.Button.MouseLeave:Connect(function()
                hovergradient.Enabled = false
                TweenService:Create(iconscale, TweenInfo.new(0.15), {Scale = 1}):Play()
            end))
            table.insert(ProjectInfra.Connections, buttondata.Button.MouseButton1Click:Connect(function() 
        ...
        InitInfo.Functions.CreateWindowControlButton = function(Data: any)
            ...
            if buttondata.Drag then
                ...
                table.insert(ProjectInfra.Connections, buttondata.Objects.Button.MouseButton1Click:Connect(function()
                ...
                if not ProjectInfra.Mobile then
                    table.insert(ProjectInfra.Connections, buttondata.Objects.Button.MouseEnter:Connect(function()
                        buttondata.Objects.Selection.ImageTransparency = 1
                    ...
                    table.insert(ProjectInfra.Connections, buttondata.Objects.Button.MouseLeave:Connect(function()
                        TweenService:Create(ActualIconScale, TweenInfo.new(0.15), {Scale = 1}):Play()
            ...
            if HasInput then
                table.insert(ProjectInfra.Connections, buttondata.Objects.Button.InputBegan:Connect(buttondata.Callbacks.InputBegan))
            end
        ...
        table.insert(ProjectInfra.Connections, UserInputService.InputEnded:Connect(function(input)
            if ProjectInfra.InputEndFunc then
                ProjectInfra.InputEndFunc(input)
            end
        end))
    
        InitInfo.Functions.Resize = function(input : InputObject)
            if InitInfo.Data.Resizing and not ProjectInfra.Config.UI.FullScreen then
                ...
                ProjectInfra.Config.UI.Size = {X = newScaleX, Y = newScaleY}
            end
        end
        ...
        InitInfo.Functions.Drag = function(mouseStart: Vector2 | Vector3 | nil, frameStart: UDim2, input: InputObject?)
            ...
            pcall(function()
                if UserCamera then
                    ...
                    ProjectInfra.Config.UI.Position = {X = newX, Y = newY}
                end
            end)
        end
        ...
        ProjectInfra.CurrntInputChangeCallback = function() end 
        table.insert(ProjectInfra.Connections, UserInputService.InputChanged:Connect(function(input)
            ProjectInfra.CurrntInputChangeCallback(input)
        end))
            ...
            Callbacks = {
                Clicked = function(self)
                    if not forcefullscreen then
                        ProjectInfra.Config.UI.FullScreen = not ProjectInfra.Config.UI.FullScreen
                    end
                    
                    if ProjectInfra.Config.UI.FullScreen or forcefullscreen then
                        if not forcefullscreen then
                            ProjectInfra.Config.UI.Position = {X = InitInfo.Objects.MainFrame.Position.X.Scale, Y = InitInfo.Objects.MainFrame.Position.Y.Scale}
                            ProjectInfra.Config.UI.Size = {X = InitInfo.Objects.MainFrame.Size.X.Scale, Y = InitInfo.Objects.MainFrame.Size.Y.Scale}
                            ProjectInfra.Config.UI.Scale = InitInfo.Objects.MainFrameScale.Scale
                        else
                            ProjectInfra.Config.UI.FullScreen = true
                        ...
                        for i,v in ProjectInfra.Corners do
                        ...
                        TweenService:Create(InitInfo.Objects.MainFrame, TweenInfo.new(0.5, Enum.EasingStyle.Exponential), {Position = UDim2.fromScale(ProjectInfra.Config.UI.Position.X, ProjectInfra.Config.UI.Position.Y), Size = UDim2.fromScale(ProjectInfra.Config.UI.Size.X, ProjectInfra.Config.UI.Size.Y)}):Play()
                        for i,v in ProjectInfra.Corners do
                            TweenService:Create(v, TweenInfo.new(0.5, Enum.EasingStyle.Exponential), {CornerRadius = UDim.new(0, 20)}):Play()
                        end
                        TweenService:Create(InitInfo.Objects.MainFrameScale, TweenInfo.new(0.5, Enum.EasingStyle.Exponential), {Scale = ProjectInfra.Config.UI.Scale}):Play()
                    end
        ...
        InitInfo.Functions.CreateWindowControlButton({
            ...
            Callbacks = {
                InputBegan = function(input)
                    if (input.UserInputType == Enum.UserInputType.MouseButton1) or (input.UserInputType == Enum.UserInputType.Touch) then
                        if ProjectInfra.Config.UI.FullScreen then 
    
                            ProjectInfra.Config.UI.FullScreen = false
                        ...
                        ProjectInfra.CurrntInputChangeCallback = function(input)
                            if (input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch) then  
                                if InitInfo.Data.Dragging and not ProjectInfra.Config.UI.FullScreen then
                ...
                Clicked = function(self)
                    ProjectInfra.ControlsVisible = not ProjectInfra.ControlsVisible
                    if ProjectInfra.ControlsVisible then
                    ...
                    if (input.UserInputType == Enum.UserInputType.MouseButton1) or (input.UserInputType == Enum.UserInputType.Touch) then
                        ...
                        ProjectInfra.InputEndFunc = function(input)
                            if (input.UserInputType == Enum.UserInputType.MouseButton1) or (input.UserInputType == Enum.UserInputType.Touch) then
                                ...
                                ProjectInfra.CurrntInputChangeCallback = function() end
                                Assets.Config.Save("UI", ProjectInfra.Config.UI)
                                ProjectInfra.InputEndFunc = function() end
                            end
                        end
                    end
                end
...
do 
    Assets.ArrayList.Init = function()
        local Data = {
            ...
            RainbowSpeed = 5000,
            Loaded = true,
            Objects = ProjectInfra.ArrayList.Objects
        }
        ...
        local TEXT_SIZE = if ProjectInfra.Mobile then 16 else 24
        
        local download = Assets.Font.Download("Product-Sans-Regular", "https://github.com/null-wtf/ProjectInfra/raw/refs/heads/main/ProjectInfra/Assets/Fonts/Product-Sans-Regular.ttf")
        ...
        local product_sans_id = Assets.Font.create_family("ProductSans", {
            {
                name = "Regular",
                weight = 400,
                file = "ProjectInfra/Assets/Fonts/Product-Sans-Regular.ttf",
            },
        })
        ...
        function Data.Functions.Toggle(visible: boolean)
            ProjectInfra.ArrayList.Objects.ArrayGui.Enabled = visible
            if not visible then
                ...
                if not Data.Connections.Rainbow then
                    ...
                    table.insert(ProjectInfra.Connections, r)
                end
            end
        end

        Holder.Parent = ProjectInfra.ArrayList.Objects.ArrayGui

        ProjectInfra.ArrayList = Data
        return Data
    end
end
...
do
    ...
    Assets.Pages.Init = function()
        ...
        InitInfo.Objects.Pageselector = Instance.new("Frame", ProjectInfra.Background.Objects.MainFrame)
        ...
        table.insert(ProjectInfra.Corners, InitInfo.Objects.PageselectorCorner)
        ...
        ProjectInfra.Background.Functions.CreateNavigationButton({
            Name = "Menu", 
            Icon = "rbxassetid://11295285432", 
            Callback = function()
                InitInfo.Functions.ToggleSelectorVisibility(not ProjectInfra.Pageselector.Objects.Pageselector.Visible)
            end
        })
    
        ProjectInfra.Pageselector = InitInfo
        return InitInfo
    end
    ...
    Assets.Pages.NewPage = function(Data)
        ...
        if not ProjectInfra.Pageselector then Assets.Pages.Init() end
        PageData.Objects.PageselectorButton = Instance.new("ImageButton", ProjectInfra.Pageselector.Objects.PageselectorButtons)
        ...
        PageData.Objects.ActualPage = Instance.new("CanvasGroup", ProjectInfra.Background.Objects.PageHolder)
        ...
        if PageData.Name == "Dashboard" then
            ...
            table.insert(ProjectInfra.Connections, MainSearchBarTextBox.Focused:Connect(function()  
            ...
            table.insert(ProjectInfra.Connections, MainSearchBarTextBox.FocusLost:Connect(function()  
            ...
            table.insert(ProjectInfra.Connections, SearchBarClear.MouseButton1Click:Connect(function()
                Search("")
                TweenService:Create(SearchBarClearScale, TweenInfo.new(0.1), {Scale = 0}):Play()
            end))
        end

        table.insert(ProjectInfra.Connections, PageData.Objects.PageselectorButton.MouseEnter:Connect(function()
        ...
        table.insert(ProjectInfra.Connections, PageData.Objects.PageselectorButton.MouseLeave:Connect(function()
        ...
        table.insert(ProjectInfra.Connections, PageData.Objects.PageselectorButton.MouseButton1Click:Connect(function()  
            ProjectInfra.Pageselector.Functions.ToggleSelectorVisibility(false)
            for i,v in ProjectInfra.Pages do
        ...
        ProjectInfra.Pages[PageData.Name] = PageData
        return PageData
    end
...
do
    Assets.Dashboard.NewTab = function(data)
        local tab = {
            ...
            Dashboard = data and data.Dashboard or ProjectInfra.Pages.Dashboard,
        ...
        if not ProjectInfra.Tabs.TabBackground then
            ProjectInfra.Tabs.TabBackground = Instance.new("ImageButton", ProjectInfra.Background.Objects.MainFrame)
            ProjectInfra.Tabs.TabBackground.AnchorPoint = Vector2.new(0.5, 0.5)
            ProjectInfra.Tabs.TabBackground.BackgroundTransparency = 1
            ProjectInfra.Tabs.TabBackground.Position = UDim2.fromScale(0.5, 0.5)
            ProjectInfra.Tabs.TabBackground.Size = UDim2.fromScale(1, 1)
            ProjectInfra.Tabs.TabBackground.Image = "rbxassetid://16286761786"
            ProjectInfra.Tabs.TabBackground.ImageTransparency = 0.5
            ProjectInfra.Tabs.TabBackground.ScaleType = Enum.ScaleType.Stretch
            ProjectInfra.Tabs.TabBackground.Visible = false
            ProjectInfra.Tabs.TabBackground.AutoButtonColor = false
            Instance.new("UICorner", ProjectInfra.Tabs.TabBackground).CornerRadius = UDim.new(0, 20)
        end

        tab.Objects.ActualTab = Instance.new("ImageButton", ProjectInfra.Tabs.TabBackground)
        ...
        tab.Objects.ActualTab.ImageColor3 = Color3.fromRGB(ProjectInfra.Config.UI.TabColor.value1, ProjectInfra.Config.UI.TabColor.value2, ProjectInfra.Config.UI.TabColor.value3)
        tab.Objects.ActualTab.ImageTransparency = ProjectInfra.Config.UI.TabTransparency
        ...
        if not ProjectInfra.Config.Game.Other.TabPos then 
            ProjectInfra.Config.Game.Other.TabPos = {}
        end
        if ProjectInfra.Config.Game.Other.TabPos[tab.Name] then
            local pos = ProjectInfra.Config.Game.Other.TabPos[tab.Name]
            if pos.X then
        ...
        tab.Functions.Drag = function(mouseStart: Vector2 | Vector3 | nil, frameStart: UDim2, input: InputObject?)
            pcall(function()
                if UserCamera then
                    ...
                    local newX = frameStart.X.Scale + (Delta.X / (Viewport.X / (ProjectInfra.Background.Objects.MainFrame.Size.X.Scale + 2.13)))
                    ...
                    for i,v in ProjectInfra.Tabs.Tabs do
                        if v.Objects and v.Objects.ActualTab then
                            ...
                            if TabPos.X.Scale > 0.9 or 0 > TabPos.X.Scale or TabPos.Y.Scale >= 0.95 or 0 > TabPos.Y.Scale then
                                if not flagged then
                                    local t = TweenService:Create(ProjectInfra.Tabs.TabBackground, TweenInfo.new(0.8, Enum.EasingStyle.Exponential), {ImageTransparency = 1})
                                    t:Play()
                                    task.spawn(function()
                                        t.Completed:Wait()
                                        task.wait(0.1)
                                        if not flagged and ProjectInfra.Tabs.TabBackground.ZIndex ~= -100 then
                                            ProjectInfra.Tabs.TabBackground.ZIndex = -100
                                        end
                                    end)
                                end
                            else
                                if v.Objects.ActualTab.Visible then
                                    TweenService:Create(ProjectInfra.Tabs.TabBackground, TweenInfo.new(0.8, Enum.EasingStyle.Exponential), {ImageTransparency = 0.5}):Play()
                                    ProjectInfra.Tabs.TabBackground.ZIndex = 1
                                    flagged = true
                                end
                            end
                        end
                    ...
                    ProjectInfra.Config.Game.Other.TabPos[tab.Name] = {X = newX, Y = newY}
                end
            end)
        end

        local InputStarting, FrameStarting = nil, nil
        table.insert(ProjectInfra.Connections, tab.Objects.DragButton.InputBegan:Connect(function(input)
            if (input.UserInputType == Enum.UserInputType.MouseButton1) or (input.UserInputType == Enum.UserInputType.Touch) then
        ...
        CloseButton.BackgroundColor3 = Color3.fromRGB(ProjectInfra.Config.UI.TabColor.value1 + 20, ProjectInfra.Config.UI.TabColor.value2 + 20, ProjectInfra.Config.UI.TabColor.value3 + 20)
        ...
        tab.Functions.ToggleTab = function(visible, anim, reopen)
            task.spawn(function()
                ...
                if visible then
                    if not reopen then
                        if not ProjectInfra.CurrentOpenTab then
                            ProjectInfra.CurrentOpenTab = {tab}
                        else
                            table.insert(ProjectInfra.CurrentOpenTab, tab)
                        end
                    end

                    ProjectInfra.Tabs.TabBackground.Visible = true
                    ...
                    if anim and ProjectInfra.Config.UI.Anim then
                            ...
                            if v.Objects and v.Objects.ActualTab then
                                ...
                                if TabPos.X.Scale > 0.9 or 0 > TabPos.X.Scale or TabPos.Y.Scale >= 0.95 or 0 > TabPos.Y.Scale then
                                    if not flagged then
                                        ...
                                        ProjectInfra.IsAllowedToHoverTabButton = false
                                    end
                                else
                                    if v.Objects.ActualTab.Visible and v ~= tab or v == tab then
                                        ProjectInfra.Tabs.TabBackground.ZIndex = 1
                                        TweenService:Create(ProjectInfra.Tabs.TabBackground, TweenInfo.new(0.8, Enum.EasingStyle.Exponential, Enum.EasingDirection.Out), {ImageTransparency = 0.5}):Play()
                                        ProjectInfra.IsAllowedToHoverTabButton = true
                                        flagged = true
                                    end
                                end
                            end
                        end
                        TweenService:Create(tab.Objects.ActualTab, TweenInfo.new(0.8, Enum.EasingStyle.Exponential, Enum.EasingDirection.Out), {ImageTransparency = ProjectInfra.Config.UI.TabTransparency}):Play()
                    ...
                    if not reopen then
                        table.remove(ProjectInfra.CurrentOpenTab, table.find(ProjectInfra.CurrentOpenTab, tab))
                    end
                    ProjectInfra.IsAllowedToHoverTabButton = false
                    ...
                    local cnt = 0 
                    for i,v in ProjectInfra.CurrentOpenTab do
                        cnt += 1
                    end
                    if 0 >= cnt then
                        ProjectInfra.Tabs.TabBackground.Visible = false
        ...
        table.insert(ProjectInfra.Connections, dashboardbuttonclickcon)


        local dashboardbuttonhovercon =  tab.Objects.DashBoardButton.MouseEnter:Connect(function()
            if not ProjectInfra.IsAllowedToHoverTabButton then
        ...
        table.insert(ProjectInfra.Connections, dashboardbuttonhovercon)
        ...
        table.insert(ProjectInfra.Connections, dashboardbuttonleavecon)
        ...
        table.insert(ProjectInfra.Connections, tabclosebuttoncon)
        ...
        table.insert(ProjectInfra.Connections, searchclearcon)
        ...
        table.insert(ProjectInfra.Connections, searchfocuslostcon)
        ...
        table.insert(ProjectInfra.Connections, searchfocuscon)
        ...
        tab.Functions.NewModule = function(data)
            ...
            Backbutton.BackgroundColor3 = Color3.fromRGB(ProjectInfra.Config.UI.TabColor.value1 + 20, ProjectInfra.Config.UI.TabColor.value2 + 20, ProjectInfra.Config.UI.TabColor.value3 + 20)
            ...
            KeyBindButton.BackgroundColor3 = Color3.fromRGB(ProjectInfra.Config.UI.KeybindColor.value1, ProjectInfra.Config.UI.KeybindColor.value2, ProjectInfra.Config.UI.KeybindColor.value3)
            KeyBindButton.BackgroundTransparency = ProjectInfra.Config.UI.KeybindTransparency
            ...
            table.insert(ProjectInfra.Connections, moduleclickcon)
            ...
            ModuleData.Functions.Toggle = function(enabled: boolean, override: boolean, notify: boolean, save: boolean, updateArray: boolean)
                ...
                local Array
                if not ProjectInfra.ArrayList.Loaded then
                    Array = Assets.ArrayList.Init()
                else
                    Array = ProjectInfra.ArrayList
                end
                ...
                if enabled then
                    if not ModuleData.Data.Enabled or override then
                        ...
                        if notify and ProjectInfra.Config.UI.Notifications then
                            Assets.Notifications.Send({
                ...
                if save then
                    ProjectInfra.Config.Game.Modules[ModuleData.Flag] = enabled
                    Assets.Config.Save(ProjectInfra.GameSave, ProjectInfra.Config.Game)
                end
            end
                ...
                if not ModuleData.Data.Keybind then
                    local suc = pcall(function()
                        if not ProjectInfra.Mobile then
                            ...
                            ProjectInfra.Background.Functions.CreateMobileButton({
                                Name = ModuleData.Name,
                    ...
                    if Save and suc then
                        ProjectInfra.Config.Game.Keybinds[ModuleData.Flag] = Bind
                        Assets.Config.Save(ProjectInfra.GameSave, ProjectInfra.Config.Game)
                    end
                end
            end
            ...
            ModuleData.Functions.UnbindKeybind = function(Save: boolean)
                if ProjectInfra.Mobile then
                    if ProjectInfra.Background.MobileButtons and ProjectInfra.Background.MobileButtons.Buttons[ModuleData.Flag.."MobileButton"] and ProjectInfra.Background.MobileButtons.Buttons[ModuleData.Flag.."MobileButton"].Functions then
                        ProjectInfra.Background.MobileButtons.Buttons[ModuleData.Flag.."MobileButton"].Functions.Destroy()
                    end
                    KeyBindButton.Text = "TAP TO BIND"
                else
                ...
                KeybindInfoText.Text = "No Keybind Set"
                ProjectInfra.Config.Game.Keybinds[ModuleData.Flag] = nil
                ModuleData.Data.Keybind = nil
            ...
            table.insert(ProjectInfra.Connections, keybindbuttonpresscon)
            table.insert(ModuleData.Connections, keybindbuttonpresscon)

            if ProjectInfra.Config.Game.Keybinds[ModuleData.Flag] then
                if ProjectInfra.Config.Game.Keybinds[ModuleData.Flag] == "Binded" and ProjectInfra.Mobile then
                    ModuleData.Functions.BindKeybind("Binded", false)
                else
                    ModuleData.Functions.BindKeybind(ProjectInfra.Config.Game.Keybinds[ModuleData.Flag], false)
                end
            end
            ...
            table.insert(ProjectInfra.Connections, keybindinputbegancon)
            ...
            table.insert(ProjectInfra.Connections, togglebuttoncon)
            ...
            table.insert(ProjectInfra.Connections, settingsbuttoncon)
            ...
            table.insert(ProjectInfra.Connections, settingsbackbuttoncon)
            ...
            ModuleData.Functions.Settings.TextBox = function(data)
                ...
                if ProjectInfra.Config.Game.TextBoxes[TextBoxData.Flag] then
                    TextBoxData.Default = ProjectInfra.Config.Game.TextBoxes[TextBoxData.Flag]                
                end
                ...
                if ProjectInfra.Mobile and TextBoxData.ToolTip == "Click to Enter A Value" then
                ...
                TextBoxData.Functions.SetValue = function(text: string, save: boolean)
                    if text and tostring(text) then
                        ...
                        ProjectInfra.Config.Game.TextBoxes[TextBoxData.Flag] = text
                        if save then
                            Assets.Config.Save(ProjectInfra.GameSave, ProjectInfra.Config.Game)
                        end
                    end
                ...
                local actualtextboxfocuslostcon = ActualTextBox.FocusLost:Connect(function() 
                    ...
                    ProjectInfra.Config.Game.TextBoxes[TextBoxData.Flag] = ActualTextBox.Text
                    Assets.Config.Save(ProjectInfra.GameSave, ProjectInfra.Config.Game)
                end)
                table.insert(ProjectInfra.Connections, actualtextboxfocuslostcon)
            ...
            ModuleData.Functions.Settings.MiniToggle = function(data)
                ...
                if ProjectInfra.Mobile and MiniToggleData.ToolTip == "Click to toggle" then
                ...
                MiniToggleData.Functions.Toggle = function(enabled, save, override)
                    ...
                    if save then
                        ProjectInfra.Config.Game.MiniToggles[MiniToggleData.Flag] = enabled
                        Assets.Config.Save(ProjectInfra.GameSave, ProjectInfra.Config.Game)
                    end
                end
                ...
                table.insert(ProjectInfra.Connections, minitoggleclickcon)
            ...
            ModuleData.Functions.Settings.Slider = function(data)
                ...
                if ProjectInfra.Config.Game.Sliders[SliderData.Flag] then
                    if typeof(ProjectInfra.Config.Game.Sliders[SliderData.Flag]) == "table" then
                        SliderData.Default = ProjectInfra.Config.Game.Sliders[SliderData.Flag]
                    elseif typeof(ProjectInfra.Config.Game.Sliders[SliderData.Flag]) == "number" then
                        SliderData.Default = {Value2 = ProjectInfra.Config.Game.Sliders[SliderData.Flag]}
                    end
                else
                ...
                SliderData.Functions.SetValue = function(value: number, save: boolean, target: number)
                    ...
                    if value then
                        ...
                        if target == 2 then
                            if ProjectInfra.Config.Game.Sliders[SliderData.Flag] and typeof(ProjectInfra.Config.Game.Sliders[SliderData.Flag]) == "table" and ProjectInfra.Config.Game.Sliders[SliderData.Flag].Value1 then
                                info = {Value1 = ProjectInfra.Config.Game.Sliders[SliderData.Flag].Value1, Value2 = value}
                            end

                        elseif target == 1 then
                            info = {Value1 = value, Value2 = SliderData.Default.Value2}
                            if ProjectInfra.Config.Game.Sliders[SliderData.Flag] and typeof(ProjectInfra.Config.Game.Sliders[SliderData.Flag]) == "table" and ProjectInfra.Config.Game.Sliders[SliderData.Flag].Value2 then
                                info = {Value1 = value, Value2 = ProjectInfra.Config.Game.Sliders[SliderData.Flag].Value2}
                            end
                        else
                        ...
                        if save then
                            ProjectInfra.Config.Game.Sliders[SliderData.Flag] = info
                            Assets.Config.Save(ProjectInfra.GameSave, ProjectInfra.Config.Game)
                        end
                    end
                end
                ...
                if SliderData.Multi then
                    ...
                    table.insert(ProjectInfra.Connections, sliderdragbuttonclickcon2)
                    table.insert(ModuleData.Connections, sliderdragbuttonclickcon2)                                
                end

                local sliderdragbuttonclickcon
                if ProjectInfra.Mobile and not SliderData.Multi then
                ...
                table.insert(ProjectInfra.Connections, sliderdragbuttonclickcon)
                ...
                table.insert(ProjectInfra.Connections, slidervaluetextchangecon)
                ...
                if SliderData.Multi then
                    ...
                    table.insert(ProjectInfra.Connections, slidervaluetextchangecon2)
            ...
            ModuleData.Functions.Settings.Dropdown = function(data)
                ...
                if not ProjectInfra.Config.Game.Dropdowns then
                    ProjectInfra.Config.Game.Dropdowns = {}
                else
                    if ProjectInfra.Config.Game.Dropdowns[DropdownData.Flag] then
                        DropdownData.Default = ProjectInfra.Config.Game.Dropdowns[DropdownData.Flag]
                    end
                end
                ...
                DropdownData.Functions.SetValue = function(NewData: string | {}, Save: boolean)
                    if NewData then
                        ...
                        if Save then
                            ProjectInfra.Config.Game.Dropdowns[DropdownData.Flag] = ReturnData
                            Assets.Config.Save(ProjectInfra.GameSave, ProjectInfra.Config.Game)
                        end
                    end
                end
                ...
                for i,v in DropdownData.Options do
                    ...
                    table.insert(ProjectInfra.Connections, ClickCon)
                    ...
                    ButtonInfo.Functions.Destroy = function()
                        for i,v in ButtonInfo.Connections do
                            ...
                            local con2 = table.find(ProjectInfra.Connections, v)
                            v:Disconnect()
                            if con1 then
                                table.remove(DropdownData.Connections, con1)
                            end
                            if con2 then
                                table.remove(ProjectInfra.Connections, con2)
                            end
                        end
                    end
                ...
                table.insert(ProjectInfra.Connections, OpenCon)
            ...
            ModuleData.Functions.Settings.Button = function(data: {Name: string, Flag: string, Description: string, ToolTip: string, Hide: boolean, Callback: any})
                ...
                if ProjectInfra.Mobile and ButtonData.ToolTip == "Click to toggle" then
            ...
            ModuleData.Functions.Settings.Keybind = function(Data: {Name: string, Description: string, Default: string, ToolTip: string, Hide: boolean, Flag: string, Callbacks: {Began: () -> (), End: () -> (), Changed: () -> ()}, Mobile: {Text: string, Default: boolean, Visible: boolean}})
                ...
                if not ProjectInfra.Config.Game.ModuleKeybinds then
                    ProjectInfra.Config.Game.ModuleKeybinds = {}
                else
                    if ProjectInfra.Config.Game.ModuleKeybinds[KeybindData.Flag] then
                        if ProjectInfra.Config.Game.ModuleKeybinds[KeybindData.Flag] == "unbinded" then
                            KeybindData.Default = ""
                        else
                            KeybindData.Default = ProjectInfra.Config.Game.ModuleKeybinds[KeybindData.Flag]
                        end
                    else
                ...
                if ProjectInfra.Mobile then
                    table.insert(ModuleData.onToggles, function(self, enabled)
                        if enabled then
                            ...
                            if ProjectInfra.Background.MobileButtons.Buttons[KeybindData.Flag.."MobileKeybind"] then
                                ProjectInfra.Background.MobileButtons.Buttons[KeybindData.Flag.."MobileKeybind"].Functions.Destroy()
                            end
                        end
                    end)
                end
                ...
                if tostring(KeybindData.Default):gsub(" ", "") ~= "" then
                    ...
                    if ProjectInfra.Mobile and tostring(KeybindData.Default) == "button" then
                    ...
                    elseif ProjectInfra.Mobile and tostring(KeybindData.Default) == "unbinded" then
                    ...
                    elseif not ProjectInfra.Mobile then
                        KeybindData.Callbacks.Changed(KeybindData, KeybindData.Default)
                ...
                local ClickCon = KeybindBox.MouseButton1Down:Connect(function()
                    if not KeybindData.Data.Keybind then
                        if ProjectInfra.Mobile then
                            ...
                            if not ProjectInfra.Background.MobileButtons.Buttons[KeybindData.Flag.."MobileKeybind"] and ModuleData.Data.Enabled then
                                ProjectInfra.Background.Functions.CreateMobileButton({
                                    Name = KeybindData.Mobile.Text,
                                    Flag = KeybindData.Flag.."MobileKeybind",
                                    Callbacks = {
                            ...
                            ProjectInfra.Config.Game.ModuleKeybinds[KeybindData.Flag] = "button"
                            ProjectInfra.Assets.Config.Save(tostring(ProjectInfra.GameSave), ProjectInfra.Config.Game)
                        else
                        ...
                        ProjectInfra.Config.Game.ModuleKeybinds[KeybindData.Flag] = nil
                        if ProjectInfra.Mobile then
                            ProjectInfra.Config.Game.ModuleKeybinds[KeybindData.Flag] = "unbinded"
                        end
                        ProjectInfra.Assets.Config.Save(tostring(ProjectInfra.GameSave), ProjectInfra.Config.Game)
                    end
                end)
                ...
                local CallbackCon = UserInputService.InputBegan:Connect(function(input)
                    ...
                    if KeybindData.Data.Binding then
                        if input.KeyCode and input.KeyCode.Name ~= "Unknown" then
                            ...
                            ProjectInfra.Config.Game.ModuleKeybinds[KeybindData.Flag] = input.KeyCode.Name
                            ProjectInfra.Assets.Config.Save(tostring(ProjectInfra.GameSave), ProjectInfra.Config.Game)
                        end
                    else
                ...
                table.insert(ProjectInfra.Connections, CallbackCon)

                table.insert(KeybindData.Connections, EndCon)
                table.insert(ProjectInfra.Connections, EndCon)

                table.insert(KeybindData.Connections, HoverCon)
                table.insert(ProjectInfra.Connections, HoverCon)
                
                table.insert(KeybindData.Connections, UnHoverCon)
                table.insert(ProjectInfra.Connections, UnHoverCon)
                ...
                KeybindData.Functions.SetValue = function(NewValue: string, save: boolean)
                    if not NewValue or NewValue == "" or NewValue == "unbinded" then
                        ...
                        if save then
                            ProjectInfra.Config.Game.ModuleKeybinds[KeybindData.Flag] = NewValue
                            ProjectInfra.Assets.Config.Save(tostring(ProjectInfra.GameSave), ProjectInfra.Config.Game)
                        end
                    end
                end
        ...
        ProjectInfra.Tabs.Tabs[tab.Name] = tab
        return tab
    end
            ...
            SectionData.Functions.NewButton = function(data)
                ...
                if ButtonData.Toggle then
                    ...
                    if ButtonData.Flag then
                        if ProjectInfra.Config.UI[ButtonData.Flag] == nil and ButtonData.Default or ProjectInfra.Config.UI[ButtonData.Flag] then
                ...
                if ButtonData.Textbox then
                    ...
                    if ButtonData.Default and typeof(ButtonData.Default) == "string" and ProjectInfra.Config.UI[ButtonData.Flag] == nil then
                        Textbox.Text = ButtonData.Default
                    end
                    if ProjectInfra.Config.UI[ButtonData.Flag] then
                        if typeof(ProjectInfra.Config.UI[ButtonData.Flag]) == "table" then
                            for i,v in ProjectInfra.Config.UI[ButtonData.Flag] do
                                Textbox.Text = Textbox.Text .. tostring(v) .. ", "
                            end
                            Textbox.Text = string.sub(Textbox.Text, 0, #Textbox.Text-2)
                        else
                            Textbox.Text = tostring(ProjectInfra.Config.UI[ButtonData.Flag])
                        end
                    end

                    table.insert(ProjectInfra.Connections, Textbox.FocusLost:Connect(function()
                ...
                table.insert(ProjectInfra.Connections, ButtonData.Objects.MainButton.MouseButton1Click:Connect(function() 
                    if ButtonData.Toggle then
...
do    
    if not ProjectInfra then 
        LocalPlayer:Kick("ProjectInfra not supported/couldn't load global environment")
        return 
    end
    ...
    Assets.Main.Uninject = function()
        ...
        ProjectInfra.Background.Objects.MainScreenGui:Destroy()
        ProjectInfra.Notifications.Objects.NotificationGui:Destroy()
        ProjectInfra.ArrayList.Objects.ArrayGui:Destroy()

        if ProjectInfra.Mobile then
            for i,v in ProjectInfra.Background.MobileButtons.Buttons do
                if v and v.Functions and v.Functions.Destroy then
                    v.Functions.Destroy()
                end
            end
        ...
        for i,v in ProjectInfra.Connections do
            v:Disconnect()
        end
        
        Assets.Main.OnUninject:Destroy()
        table.clear(getgenv().ProjectInfra)
        getgenv().ProjectInfra = nil
    end

    local cantogglewithkeybind = true
    Assets.Main.Load = function(file)
        if not ProjectInfra.Background then
            ProjectInfra.Background = Assets.MainBackground.Init()
        end

        if not ProjectInfra.Dashboard then
            ProjectInfra.Dashboard = Assets.Pages.NewPage({
                Name = "Dashboard",
            ...
            Assets.Dashboard.NewTab({
                ...
                Dashboard = ProjectInfra.Dashboard
            })
            ...
            MainSettings.Functions.NewButton({Name = "Change Keybind", Callback = function(self)
                ...
                changecon = UserInputService.InputBegan:Connect(function(input)
                    if input and input.KeyCode.Name ~= "Unknown" then
                        ...
                        ProjectInfra.Config.UI.ToggleKeyCode = input.KeyCode.Name
                        Assets.Config.Save("UI", ProjectInfra.Config.UI)
                        task.wait(1)
            ...
            ThemeSettings.Functions.NewButton({Name = "TabColor", Textbox = true, Flag = "TabColor", Default = "70, 70, 70", Callback = function(self, value)
                ...
                if #split == 3 then
                    ...
                    if tonumber(v1) and tonumber(v2) and tonumber(v3) then
                        ProjectInfra.Config.UI.TabColor = {value1 = tonumber(v1), value2 = tonumber(v2), value3 = tonumber(v3)}
                        Assets.Config.Save("UI", ProjectInfra.Config.UI)
                        for i,v in ProjectInfra.Tabs.Tabs do
            ...
            ThemeSettings.Functions.NewButton({Name = "TabTransparency", Textbox = true, Flag = "TabTransparency", Default = "0.1", Callback = function(self, value)
                if tonumber(value) then
                    ProjectInfra.Config.UI.TabTransparency = tonumber(value)
                    for i,v in ProjectInfra.Tabs.Tabs do
                        v.Objects.ActualTab.ImageTransparency = ProjectInfra.Config.UI.TabTransparency
                    end
                    Assets.Config.Save("UI", ProjectInfra.Config.UI)
                end
            end})
            ThemeSettings.Functions.NewButton({Name = "KeybindColor", Textbox = true, Flag = "KeybindColor", Default = "85, 89, 91", Callback = function(self, value)
                ...
                if #split == 3 then
                    local v1, v2, v3 = split[1]:gsub(" ", ""), split[2]:gsub(" ", ""), split[3]:gsub(" ", "")
                    if tonumber(v1) and tonumber(v2) and tonumber(v3) then
                        ProjectInfra.Config.UI.KeybindColor = {value1 = tonumber(v1), value2 = tonumber(v2), value3 = tonumber(v3)}
                        Assets.Config.Save("UI", ProjectInfra.Config.UI)
                        for i,v in ProjectInfra.Tabs.Tabs do
            ...
            ThemeSettings.Functions.NewButton({Name = "KeybindTransparency", Textbox = true, Flag = "KeybindTransparency", Last = true, Default = "0.015", Callback = function(self, value)
                if tonumber(value) then
                    ProjectInfra.Config.UI.KeybindTransparency = tonumber(value)
                    Assets.Config.Save("UI", ProjectInfra.Config.UI)
                    for i,v in ProjectInfra.Tabs.Tabs do
                        if v.Objects.ActualTab:FindFirstChildWhichIsA("TextButton") then
            ...
            Assets.Functions.LoadFile("ProjectInfra/Games/"..file..".lua", "https://raw.githubusercontent.com/null-wtf/ProjectInfra/refs/heads/main/ProjectInfra/Games/"..file..".lua")
            Assets.Functions.LoadFile("ProjectInfra/Premium/Games/"..file..".lua", "https://raw.githubusercontent.com/null-wtf/ProjectInfra/refs/heads/main/ProjectInfra/Premium/Games/"..file..".lua")
            Assets.Config.Load(file, "Game")
            return {Background = ProjectInfra.Background, Dashboard = ProjectInfra.Dashboard, Settings = Settings}
        else
            Assets.Functions.LoadFile("ProjectInfra/Games/"..file..".lua", "https://raw.githubusercontent.com/null-wtf/ProjectInfra/refs/heads/main/ProjectInfra/Games/"..file..".lua")
            Assets.Functions.LoadFile("ProjectInfra/Premium/Games/"..file..".lua", "https://raw.githubusercontent.com/null-wtf/ProjectInfra/refs/heads/main/ProjectInfra/Premium/Games/"..file..".lua")
            Assets.Config.Load(ProjectInfra.GameSave, "Game")
            return {Background = ProjectInfra.Background, Dashboard = ProjectInfra.Dashboard}
        end
    end
    ...
    Assets.Main.ToggleVisibility = function(visible: boolean)
        do
            ...
            if not ProjectInfra.Config.UI.Anim then
                ProjectInfra.Background.Objects.MainFrame.Visible = visible
                if visible then
                    ProjectInfra.Background.Objects.MainFrame.BackgroundTransparency = 0.1
                    ProjectInfra.Background.Objects.MainFrame.ImageTransparency = 0.8
                    ProjectInfra.Background.Objects.MainFrameScale.Scale = 1
                    ProjectInfra.Background.Objects.WindowControls.GroupTransparency = 0.4
                end
                return
            end
            ...
            if visible then
                if ProjectInfra.Mobile then
                    ProjectInfra.Background.Objects.ToggleButton.Visible = false
                end
                ProjectInfra.Background.Objects.MainFrame.Visible = true
                ProjectInfra.Background.Objects.DropShadow.Visible = true
                
                ProjectInfra.Background.Objects.MainFrame.BackgroundTransparency = 1
                ProjectInfra.Background.Objects.MainFrame.ImageTransparency = 1
                ProjectInfra.Background.Objects.MainFrameScale.Scale = 1
                ProjectInfra.Background.Objects.WindowControls.GroupTransparency = 1

                table.insert(RunningTweens, TweenService:Create(ProjectInfra.Background.Objects.MainFrame, TweenInfo.new(1.2, Enum.EasingStyle.Exponential, Enum.EasingDirection.Out), {BackgroundTransparency = 0.1, ImageTransparency = 0.8}))
                table.insert(RunningTweens, TweenService:Create(ProjectInfra.Background.Objects.WindowControls, TweenInfo.new(0.8, Enum.EasingStyle.Exponential, Enum.EasingDirection.Out), {GroupTransparency = 0.4}))
                ...
                if ProjectInfra.CurrentOpenTab then
                    for i,v in ProjectInfra.CurrentOpenTab do
                        if v.Functions then
                            task.wait(0.015)
                ...
                if ProjectInfra.Notifications.Active.discordnoti then
                    ProjectInfra.Notifications.Active.discordnoti.Functions.Remove(true)
                end
                if ProjectInfra.Mobile then
                    ProjectInfra.Background.Objects.ToggleButton.Visible = true
                end
                ...
                if ProjectInfra.Tabs.TabBackground.Visible then
                    ProjectInfra.Tabs.TabBackground.Visible = false
                end

                table.insert(RunningTweens, TweenService:Create(ProjectInfra.Background.Objects.MainFrame, TweenInfo.new(1.2, Enum.EasingStyle.Exponential, Enum.EasingDirection.Out), {BackgroundTransparency = 1, ImageTransparency = 1}))
                table.insert(RunningTweens, TweenService:Create(ProjectInfra.Background.Objects.WindowControls, TweenInfo.new(0.8, Enum.EasingStyle.Exponential, Enum.EasingDirection.Out), {GroupTransparency = 1}))

                for i,v in RunningTweens do v:Play() end

                if ProjectInfra.Pageselector.Objects.Pageselector.Visible then
                    ProjectInfra.Pageselector.Objects.Pageselector.Visible = false
                    table.insert(Restore, ProjectInfra.Pageselector.Objects.Pageselector)
                end

                ProjectInfra.Background.Objects.NavigationButtons.Visible = false
                table.insert(Restore, ProjectInfra.Background.Objects.NavigationButtons)
                ProjectInfra.Background.Objects.WindowControls.Visible = false
                table.insert(Restore, ProjectInfra.Background.Objects.WindowControls)
                ...
                ProjectInfra.Background.Objects.DropShadow.Visible = false

                task.delay(0.8, function()
                    ProjectInfra.Background.Objects.MainFrame.Visible = Open
                end)
            end
        end
    end
    table.insert(ProjectInfra.Connections, UserInputService.InputBegan:Connect(function(input, gameProcessed)
        if gameProcessed and UserInputService:GetFocusedTextBox() or not cantogglewithkeybind then return end
        if input.KeyCode.Name == ProjectInfra.Config.UI.ToggleKeyCode then
            Assets.Main.ToggleVisibility(not ProjectInfra.Background.Objects.MainFrame.Visible)
        end
    end))

end

if ProjectInfra.Premium then
    ProjectInfra.Assets = Assets
    return Assets
end
